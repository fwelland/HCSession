...
public class SessionFailoverFilter implements Filter
{
	...
	// Write code here to process the request and/or response before
        ...
        if(null!=haSessionID)
        {
            log("located existing session id ["+ haSessionID + "]"); 
        }
        else
        {
            haSessionID = generateSessionId();
            addSessionCookie((HttpServletRequest)request, haSessionID,(HttpServletResponse) response);
            log("ADDING a new session if ["+ haSessionID + "]");
        }
    }
